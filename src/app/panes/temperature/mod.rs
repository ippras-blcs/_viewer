...
impl Pane {
    ...
    fn top(&mut self, ui: &mut Ui, state: &mut State) -> Response {
        let mut response = ui.heading(NOTE_PENCIL).on_hover_localized("Temperature");
...
impl Pane {
    ...
    fn metadata_window(&mut self, ui: &mut Ui, state: &mut State) {
        Window::new(format!("{TAG} Temperature metadata"))
    ...
    fn settings_window(&mut self, ui: &mut Ui, state: &mut State) {
        if let Some(inner_response) =
            Window::new(format!("{SLIDERS_HORIZONTAL} Temperature settings"))
